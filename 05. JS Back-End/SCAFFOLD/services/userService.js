...
async function register(username, password) { // тук след регистрация да видя на къде се редиректва в задачата
    ...
    if (existingUsername) {                                     // Ако намерим съвпадение
        throw new Error('The username or email provided is already in use!');   // Хвърляме грешка
    }
    ...
    const user = await User.create({    //Създаваме нов потребител в базата данни с username, email и hashedPassword 
        username,
        hashedPassword,
    });
...
function createSession({ _id, username }) { //това е user обекта който сме направили по горе
