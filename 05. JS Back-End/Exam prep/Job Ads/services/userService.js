...
async function register(email, skills, password) { // тук след регистрация да видя на къде се редиректва в задачата
    ...
    if (existingEmail) {                                        // Ако намерим съвпадение
        throw new Error('Email or password already exists!');   // Хвърляме грешка
    }
...
function createSession({ _id, email, skills }) { //това е user обекта който сме направили по горе
