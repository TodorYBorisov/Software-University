...
createController.post('/',
    ...
    body('age')
        ...
        .isInt({ min: 1, max: 100 }).withMessage('Age must be a positive number between 1 and 100'),
    body('description')
        .notEmpty().withMessage('Description is required').bail()
        .isLength({ min: 5, max: 50 }).withMessage('Description must be between 5 and 50'),
    body('location')
        .notEmpty().withMessage('Location is required').bail()
        .isLength({ min: 5, max: 50 }).withMessage('Location must be between 5 and 50'),
    body('imageUrl')
