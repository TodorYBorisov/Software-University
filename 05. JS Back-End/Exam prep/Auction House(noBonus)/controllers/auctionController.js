...
const { create, getAll, getById, deleteById, updateById, addToWishList, bid, getClosedAuctions, closed } = require('../services/auctionService');
...
auctionController.get('/closed', async (req, res) => {
    try {
        ...
        const closedAuctions = await getClosedAuctions();

        console.log(closedAuctions);

        // if (closedAuctions.bidder.length > 0) {
