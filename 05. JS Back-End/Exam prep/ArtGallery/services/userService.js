...
async function register(username, address, password) { // тук след регистрация да видя на къде се редиректва в задачата
    ...
    if (existingUsername) {                                     // Ако намерим съвпадение
        throw new Error('Username or address already exists!');   // Хвърляме грешка
    }
    ...
    // }
    if (password.length < 3) {                                   // Валидираме дължината на паролата
...
async function login(username, password) {
    ...
    if (password.length < 3) {                      // Ако има валидация за дължина на паралота
        throw new Error('Password is too short!'); // Хвърляме грешка
    }
...
function createSession({ _id, username, address }) { //това е user обекта който сме направили по горе
    ...
    const token = jwt.sign(payload, SECRET, {expiresIn: '2d'}); // Връщаме подписан токена, ако искаме да изтича добавяме 
    return token;                           //{expiresIn: '2d'} ако искаме да изтича токена го подаваме след secter
}
