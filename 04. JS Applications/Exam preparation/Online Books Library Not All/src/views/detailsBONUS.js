...
import { deleteBook, getByBookIdandUserId, getById, getLikesByBookId } from '../data/data.js';
...
export async function detailsBONUS(ctx) {
    const bookId = ctx.params.id; //взимаме id от контекста и си сетваме наше bookId
    const dataBook = await getById(bookId); // правяим заявка за взияки книги по bookId
    ...
    const myLikes = await getByBookIdandUserId(bookId, userId); //това са нашите лайкове
